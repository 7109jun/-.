...
<head>
  <meta charset="UTF-8" />
  <title>JOCP V5.4 - Optimized & Scoped</title>
  <style>
    * { box-sizing: border-box; }
    ...
    .editor-container { display: flex; gap: 0; height: 80vh; border: 1px solid #ced4da; border-radius: 8px; overflow: hidden; background: #ffffff; }
    .panel { overflow: auto; position: relative; }
    #editor-panel { flex: 1; min-width: 200px; }
    #preview-panel { flex: 1; min-width: 200px; }
    ...
    .resizer { width: 8px; background: #e9ecef; cursor: col-resize; transition: background 0.2s; flex-shrink: 0; }
    .resizer:hover, .resizer.active { background: #4a9eff; }
    
    #preview { padding: 25px; height: 100%; overflow-y: auto; }
...
<body>
  ...
  <div class="editor-container">
    <div class="panel" id="editor-panel">
      <textarea id="jocp-input">~meta
제목: JOCP V5.4 - 최적화 및 CSS 스코핑
저자: 개발자
날짜: 2026-07-18
태그: JOCP, 최적화, CSS스코핑
설명: scriptStore 버그 수정, render 중복 행 방지, 게임 CSS 범위 제한, 화면 크기 조절 기능 추가
~
...
아래 게임의 CSS는 게임 컨테이너 내부에만 적용됩니다.
...
<div class="game-box">
  <div class="ball"></div>
  <p class="info">이 텍스트는 게임 내부 타일이 적용됩니다</p>
</div>
css:
.game-box { 
  width: 100%; 
...
~
  ...
  <script>
    const JOCP = (function() {
      ...
      // 모듈 레벨 변수 (IIFE 반환 전에 안정적으로 접근 가능)
      let scriptStore = [];
      let metaData = {};
      ...
      // CSS 스코핑: 택자에 게임 ID를 추가하여 외부 침범 방지
      function scopeCSS(css, scopeId) {
        return css.replace(/([^{}]+)\{/g, (match, selectors) => {
          const scoped = selectors.split(',').map(s => {
            const trimmed = s.trim();
            if (!trimmed || trimmed.startsWith('@')) return trimmed; // @media 등 제외
      ...
      function parseGameTemplate(lines, startIdx) {
        ...
        // CSS 스코핑: 선택자에 게임 ID를 추가하여 외부 침범 방지
        const scopedCSS = scopeCSS(cssContent, gameId);
      ...
      return {
        ...
        parse: function(text) { scriptStore = []; metaData = {}; return parseBlock(text.split('\n')); },
        ...
        render: function(inputId, previewId) {
          ...
          const update = () => { preview.innerHTML = this.parse(newInput.value); };
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      document.addEventListener('mousemove', (e) => {
        ...
        const editorPercent = (offset / totalWidth) * 100;
        const previewPercent = 100 - editorPercent;
        
        if (editorPercent > 20 && previewPercent > 20) {
      ...
      // JOCP 초기 렌더링 (한 번만 실행)
      JOCP.render('jocp-input', 'preview');
    });
  </script>
</body>
</html>```
